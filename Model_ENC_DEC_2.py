...
class ENC_DEC(object):
    ...
    def set_mask(self):
        ...
        ndim = self.layers[0].x_mask.ndim
    ...
    def set_input_y(self):
        for l in self.layers:
            if hasattr(l, 'y'):
                ndim = l.y.ndim
                dtype=l.y.dtype
                self.layers[0].y = ndim_tensor(ndim,dtype)
                break

    def get_input_y(self):
        if not hasattr(self.layers[0], 'y'):
            self.set_input_y()
        return self.layers[0].get_input_y()  
    ...
    def build(self):
        ...
        self.params+=[self.W_hi, self.b_hi,self.W_hy, self.b_hy]        
    ...
    def compile(self,optimizer='Adam',loss='nll_multiclass_3d'):      
        ...
        self.x_mask = self.get_mask()
        
        self.X_test = self.get_input(train=False)
        #self.X_test_mask = self.get_mask()
        
        self.y_mask = T.matrix(name = 'y', dtype = 'int32')

        self.Y_train_input = self.get_input_y()
        #self.Y_train_mask = self.get_mask_y()
        
        self.Y_test_input = self.get_input_y()
        #self.Y_test_mask = self.get_mask_y()        
        ...
        if type(self.X_train) == list:
            train_ins = self.X_train + self.x_mask + self.Y_train_input + self.y_mask
            test_ins = self.X_test + self.x_mask + self.Y_test_input + self.y_mask
            predict_ins = self.X_test + self.x_mask + next_y + next_h
        ...
        train_loss=self.loss(self.Y_train_input, self.y_mask ,self.y_train)
        test_loss=self.loss(self.Y_test_input, self.y_mask ,self.y_test)
        ...
        #idx=T.lscalar() 
        if self.shared_emb:                                    
            self.get_embedding = lambda idx: self.layers[0].W[idx] 
        else:
            self.get_embedding = lambda idx: self.layers[0].W_multi[idx]
    ...
    def get_output(self,train):
        ...
        emb=self.layers[0].get_multi_output() 
        
        proj=self.layers[-1].get_output(emb,self.y_mask,init_state,train)
